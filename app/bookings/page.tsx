    ...
    <div className="space-y-8 animate-in slide-in-from-right-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Review, approve, and track vehicle assignments</p>
        </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-[0_8px_40px_rgb(0,0,0,0.03)] border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex space-x-6 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            {['All Status', 'Pending', 'Approved', 'Completed'].map((tab, i) => (
          ...
          <table className="min-w-full">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-8 py-5 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">Passenger</th>
                <th className="px-8 py-5 text-left text-[11px] font-black text-slate-400 uppercase tracking-widest">Destination & Reason</th>
              ...
              {bookings.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-8 py-24 text-center">
                    ...
                    <p className="text-slate-500 font-medium">Recorded bookings will appear here.</p>
                  </td>
                </tr>
              ) : (
                bookings.map((b: any) => (
                  <tr key={b.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] font-bold text-slate-400 uppercase">Employee ID: 00{b.id}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-6 max-w-xs">
                      ...
                      <div className="text-xs text-slate-400 italic line-clamp-1">"{b.reason || 'No reason provided'}"</div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        <button className="p-2 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 rounded-lg transition-colors" title="Approve">
                          <CheckCircle2 className="w-5 h-5" />
                        </button>
                        <button className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors" title="Reject">
